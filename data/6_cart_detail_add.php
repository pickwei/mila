...
if($row){ //购物车存在
  ...
  $sql = "INSERT INTO hupo_cart VALUES(NULL, $uid)";
  mysqli_query($conn,$sql);
  $cid = mysqli_insert_id($conn);
}

//判断购物车详情表中是否已经存在该商品记录
$sql = "SELECT did,count FROM hupo_cart_detail WHERE cartId=$cid AND productId=$pid";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_assoc($result);
if($row){   //之前曾经购买过该商品，则购买数量+1
  $count = intval($row['count']);
  $count++;
  $sql = "UPDATE hupo_cart_detail SET count=$count WHERE cartId=$cid AND productId=$pid";
  mysqli_query($conn,$sql);
}else {     //之前从未购买过该商品，则添加购买记录，购买数量为1
  $count = 1;
  $sql = "INSERT INTO hupo_cart_detail VALUES(NULL,$cid, $pid, $count)";
  mysqli_query($conn,$sql);
}
